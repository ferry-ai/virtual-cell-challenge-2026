"""Network smoothing of MEASURED targets: do a target's STRING partners sharpen its transferred effect?
...
PDS proxy (analyze.pds_proxy), paired bootstrap over targets. Proxies, not VCC scores.
    ...
    scripts/py.cmd reports/rete_2026-09-26/network_smoothing.py \
        --universe C:/Users/ferra/vcc2026-data/processed/universe_k562_2026-09-26 --out reports/rete_2026-09-26/r1
"""
...
def main() -> None:
    ...
    info = pd.read_csv(args.string_info, sep="\t", usecols=[0, 1], compression="gzip")
    sym = dict(zip(info.iloc[:, 0], info.iloc[:, 1]))
    links = pd.read_csv(args.string_links, sep=" ", compression="gzip")
    links = links[links["combined_score"] >= 700]
    links = pd.DataFrame({"a": links["protein1"].map(sym), "b": links["protein2"].map(sym)}).dropna()
    first = np.load(sorted(args.universe.glob("k562_*.npz"))[0], allow_pickle=False)
    gene_cols = np.flatnonzero(np.isfinite(first["raw"]).mean(axis=0) > 0.5)
    del first
    net, n_with = partner_means(args.universe, panel, links, gene_cols, axis.size)
    del links
    ...
    with (args.out / "measurements.json").open("x", encoding="utf-8") as f:
        json.dump({"stage": "rete_2026-09-26/network_smoothing.py", "claim_type": "effect-space proxies; not VCC scores",
