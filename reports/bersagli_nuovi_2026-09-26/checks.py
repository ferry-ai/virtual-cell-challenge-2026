"""Two checks of r1 before reading it: is the linear model's null a bug, and can STRING help cis?
...
2. STRING scaled before adding the cis head: lambda x string700 + cis, lambda = 0.1, 0.25, 0.5,
   on the r1 held-out sources (the sum in r1 was lambda = 1 and lost to cis alone out of K562).
    ...
    scripts/py.cmd reports/bersagli_nuovi_2026-09-26/checks.py \
        --universe C:/Users/ferra/vcc2026-data/processed/universe_k562_2026-09-26 --out reports/bersagli_nuovi_2026-09-26/r2
"""
...
def main() -> None:
    ...
    with (args.out / "measurements.json").open("x", encoding="utf-8") as f:
        json.dump({"stage": "bersagli_nuovi_2026-09-26/checks.py", "claim_type": "effect-space proxies; not VCC scores",
