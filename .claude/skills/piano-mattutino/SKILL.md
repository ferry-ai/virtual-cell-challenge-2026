...
`CLAUDE.md` vale per intero: disciplina dell'evidenza, tipi di affermazione, registro,
...
- `docs/PIANO_IMPLEMENTATIVO_<AAAA-MM-GG>.md` (workflow 1): intestazione con data, ora,
  responsabile («lead scientist, agente Claude»), stato «proposta», link al workflow 2 e
...
3. Nessun checkpoint per un piano: è una proposta. Lo si scrive solo se nel frattempo è
   successo qualcosa della lista di `CLAUDE.md` e nessuno l'ha ancora registrato; in quel
