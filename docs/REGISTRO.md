...
| `reports/pulizia_2026-09-19/` | attuale | — | Resoconto della pulizia del codice del 19 settembre sul branch `refactor/pulizia`: righe prima e dopo per cartella, i quattro commit con la prova di equivalenza su file reali, le duplicazioni lasciate stare perché le copie sono andate alla deriva, e le proposte che richiedono il proprietario (catena dei cicli, orchestratore, oracle: 17.075 righe che il controllo documentale non permette di cancellare perché i checkpoint immutabili le citano). Non cambia nessun risultato e non adotta niente | — |
