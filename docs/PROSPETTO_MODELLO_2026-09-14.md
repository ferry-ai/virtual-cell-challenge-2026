...
[models.py](../src/vcc2026/models.py) trasferisce la firma di ciascun bersaglio
misurata in K562, con shrinkage per gene e ampiezza globale:

`delta_pred = alpha * prior_sd² / (prior_sd² + SE²) * delta_K562`.

La configurazione in [trials.yaml](../configs/trials.yaml) usa alpha calibrato
su K562 → RPE1, poi applicato invariato ad A/B/C. Il basale del contesto entra nella
conversione in conteggi in [inference.py](../src/vcc2026/inference.py), ma la regola
...
Il [JSON del server](../reports/trial_2026-09-13/status_PNn227rxP3bVByS37W41.json)
...
Stato locale da [registry](../configs/sources.yaml) e [roadmap](ROADMAP.md), fotografie
